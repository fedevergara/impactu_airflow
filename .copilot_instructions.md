...
- Do not make automatic commits to the repository under any circumstances.
- Suggest changes and propose commit messages when appropriate, but only perform a commit when the user explicitly requests it.

Note: The assistant may create patches and show exactly what changes it proposes; the actual commit must wait for explicit user confirmation.
